...
namespace SportClub.DAL.Repositories
{
    public class TrainingGroupRepository : ITrainingGroupRepository
    {
        ...
        {
            return await db.TrainingsGroup.Include((p) => p.Room).Include((p) => p.users).Include((p) => p.Coach)/*.Include((p) => p.Time)*/.ToListAsync();
        ...
        {
            return await db.TrainingsGroup.Where((p) => p.users.Contains(u)).Include((p) => p.users).Include((p) => p.Coach)/*.Include((p) => p.Speciality)*/.Include((p) => p.Time).ToListAsync();
        ...
        {
            return await db.TrainingsGroup.Include((p) => p.Room).Include((p) => p.users).Include((p) => p.Coach)/*.Include((p) => p.Speciality).Include((p) => p.Time)*/.FirstOrDefaultAsync(m => m.Id == id);
